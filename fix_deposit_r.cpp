...
/* ---<><><>---------------------------
 ...
 If the deposit type is given as -1 then fix_deposit will look for the types keyword
 and a list of types to randomly select among
 for that added
 int poly_flag - alert the program to this option
 int nptypes - the number of types in the list
 int array type_list - the list of types to select from
  ...
  fixrigid(nullptr), fixshake(nullptr), random(nullptr)
{
  ...
  restart_global = 1;
  time_depend = 1;
  poly_flag = 0; // ---<><><>--- deposition from types selected randomly from type_list
  nptypes = 0;   // ---<><><>--- number of types in type_list - counted in options routine
  ...
  //---<><><>--- if type = -1 (to allow random choice from type_list)
  //             then set poly_flag and create and initialize type_list
  if (ntype == -1) {
    poly_flag = 1;
    type_list = nullptr;
    memory->create(type_list,atom->ntypes,"deposit:type_list");
    for (int i = 0; i < atom->ntypes; i++) type_list[i] = 0;
  }
  ...
  // ---<><><>--- if ntype = -1, the keyword types is required followed by a list of types
  if (poly_flag == 1 && nptypes == 0) error->all(FLERR,"Poly flag but no type list in fix deposit command");

  // error check on type
  if (mode == ATOM && poly_flag == 0 && (ntype <= 0 || ntype > atom->ntypes))
...
FixDeposit::~FixDeposit()
{
  ...
  // ---<><><>--- get rid of list used for random type selection
  if (poly_flag == 1) memory->destroy(type_list);
}
...
void FixDeposit::pre_exchange()
{
  ...
  while (attempt < maxattempt) {
    ...
    //---<><><>--- if poly_flag randomly select ntype from type_list (else ntype was input parameter)
    if (poly_flag == 1) {
    int pndx = static_cast<int> (nptypes*random->uniform());
      ntype = type_list[pndx];
    }
...
void FixDeposit::options(int narg, char **arg)
{
  ...
  // ---<><><>--- recognize types key word - create type_list, record number of types in nptypes,
  //              and check that their values are between 0 and atom->ntypes
  ...
  while (iarg < narg) {
    if (strcmp(arg[iarg],"region") == 0) {
      ...
      if (ntype != -1) error->all(FLERR,"Illegal fix deposit command");
      if (iarg+3 > narg) error->all(FLERR,"Illegal fix deposit command");
      iarg++;
      while (iarg < narg) {
        if (isalpha(arg[iarg][0])) break;
        if (nptypes >= atom->ntypes) error->all(FLERR,"Illegal fix deposit command");
        type_list[nptypes] = utils::numeric(FLERR,arg[iarg],false,lmp);
        if (type_list[nptypes] < 0 || type_list[nptypes] > atom->ntypes)error->all(FLERR,"Illegal atom type in fix deposit type list");
        nptypes++;
        iarg++;
      }
    } else error->all(FLERR,"Illegal fix deposit command");
